        ...
        try:
            ...
            if image in image_files:
                ...
                while num < len(entities):
                    ...
                    reflect_text = ''
                    Number_of_errors = 0
                    while number_ref == 0 and correct_ref == 0:
                        Number_of_errors+=1
                        logger.info(f'进行对实体{entity["name"]}的第{Number_of_errors}次提取')
                        ...
                        # ========== 对图片中提取出来的实体进行反思行为 ==========
                        # 1、进行实体提取数量的反思
                        number_ref = ImageEntityExtractor.reflect_number(image_path=image_input_path + image, text=text, entity=entity["name"], bnd=bnd, description=description)
                        if number_ref == 0:
                            reflect_text = f'the number of {entity["name"]} is not {len(bnd)}'
                            continue
