...
def remove_duplicates(data):
    ...
    result = []
    for item in data:
        # 将name和bnd组合成一个元组作为唯一标识
        identifier = (item['name'], tuple(item['bnd']))
        if identifier not in seen:
            seen.add(identifier)
            result.append(item)
    return result
...
def main():
    ...
    for idx, item in tqdm(texts.items(), desc="正在抽取实体"):
        ...
        try:
            ...
            # ========== 对提取的文本实体进行文本实体反思 ==========
            # Text_reflect_ref = APIextractor.reflect_text_entities(text, entities)
            ...
            if image in image_files:
                ...
                while num < len(entities):
                    entity = entities[num]
                    num += 1
                    logger.info(f"现在提取图片中的实体: {entity}")
                    if entity["label"] == "location" or entity["label"] == "other":
                    ...
                    # ========== 使用API对所提取出的实体进行可具象化判断 ==========
                    if entity["label"] == "other":
                        text_judge = APIextractor.text_pre_judge(text=text, entity_name=entity["name"], entity_label=entity["label"])
                        if text_judge == 0:
                            logger.error(f"实体 {entity['name']} 不是可提取实体，跳过处理。")
                            entity['bnd'] = None
                            if not results.__contains__(idx):
                                results[idx] = []
                            results[idx].append(entity)
                    ...
                    #
                    #
                    # 先判断是否存在并提取数量，再提取对应的图片实体内容。如何对文本实体的提取进行反思成为关键问题。
                    #
                    #
                    # ========== 使用视觉模型进行预判断 ==========
                    NEED_REFLECT = False
                    pre_judge_num = ImageEntityExtractor.pre_judge(image_input_path + image, text, entity["name"], entity["label"], description)
                    if pre_judge_num == 0:
                        NEED_REFLECT = True
                        reflect_text = f'{entity} is not exist in image!'
                    ...
                    reflect_text = ''
                    pre_judge_num = 1
                    prompt = ImageEntityExtractor.generate_prompt(text, entity["name"], entity["label"], description, pre_judge_num)
                    bnd = ImageEntityExtractor.extract_entities_from_image(image_input_path + image, prompt + reflect_text)
                    bnd = remove_duplicates(bnd)
                    index = 0
                    if bnd == []:
                        logger.error(f"确定未能在图片中找到实体 {entity['name']}，跳过处理。")
                        entity['bnd'] = None
                        if not results.__contains__(idx):
                            results[idx] = []
                        results[idx].append(entity)
                    ...
                    for i in bnd:
                        if i["name"] != entity["name"]:
                            logger.error(f"提取的实体名称 {i['name']} 与预期实体名称 {entity['name']} 不匹配，跳过处理。")
                            continue
                        ...
                        # NEED_REFLECT = True
                        if NEED_REFLECT:
                            correct_ref = ImageEntityExtractor.reflect_correct(text=text, image_path=image_input_path + image, entity=entity, description=description)
                            if correct_ref == 0:
                        ...
                        if not results.__contains__(idx):
                            results[idx] = []
                        results[idx].append(entity)
                        logger.success(f"完成对{idx}样本{entity}的处理")
